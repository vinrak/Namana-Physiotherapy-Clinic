    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 bg-slate-50 text-slate-800">
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-3xl border border-sky-100 shadow-xs">
          <div className="flex items-center gap-3 min-w-0">
            <div className="p-3 bg-sky-100 rounded-2xl text-sky-700 shrink-0">
              <FileSpreadsheet className="w-6 h-6 text-emerald-600" />
            </div>
            <div className="min-w-0">
              <h2 className="text-lg font-bold text-sky-950 truncate">Google Sheets Synchronization &amp; Cloud Backup</h2>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {cleanId ? (
              ...
              <div className="px-3 py-1.5 bg-slate-100 text-slate-600 rounded-xl text-xs font-semibold">
                No Sheet Configured
              </div>
            )}
          </div>
        ...
        {statusMsg && (
          ...
          >
            ...
            {statusMsg.type === 'info' && <RefreshCw className="w-4 h-4 text-sky-600 shrink-0 mt-0.5 animate-spin" />}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-6 items-stretch">
          ...
          <div className="bg-white p-5 sm:p-6 rounded-3xl border border-sky-100 shadow-xs flex flex-col justify-between space-y-4">
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                <h3 className="text-sm font-bold text-sky-950 uppercase tracking-wide flex items-center gap-2 min-w-0">
                ...
                {cleanId && (
                  <button
                    ...
                    className="flex items-center gap-1 text-xs font-bold text-sky-600 hover:text-sky-800 hover:underline cursor-pointer shrink-0"
                  >
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700">Spreadsheet ID or Full URL</label>
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono text-slate-800 placeholder-slate-400 focus:bg-white focus:border-sky-500 outline-none shadow-2xs"
                  />
                </div>
                {cleanId && cleanId !== sheetInput.trim() && (
                  <p className="text-[11px] text-emerald-700 font-mono bg-emerald-50 px-3 py-1 rounded-lg border border-emerald-100 truncate">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-3 px-3.5 bg-sky-600 hover:bg-sky-700 text-white rounded-xl text-xs font-bold transition-all cursor-pointer shadow-xs disabled:opacity-40"
                >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-3 px-3.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 rounded-xl text-xs font-bold transition-all cursor-pointer shadow-2xs"
                  title="Copies entire patient database formatted for Ctrl+V paste into Google Sheet cell A1"
                >
                  {copiedClipboard ? <Check className="w-4 h-4 text-emerald-600 shrink-0" /> : <Copy className="w-4 h-4 text-emerald-600 shrink-0" />}
              ...
              <div className="flex justify-end">
                <button
                  ...
                  disabled={isPulling || (!cleanId && !webhookUrl)}
                  className="text-[11px] font-semibold text-slate-500 hover:text-rose-600 hover:underline cursor-pointer transition-colors"
                >
                  Replace Entire Local Directory with Sheet Data
                </button>
              </div>
          ...
          <div className="bg-white p-5 sm:p-6 rounded-3xl border border-sky-100 shadow-xs flex flex-col justify-between space-y-4">
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-emerald-50 text-emerald-800 border border-emerald-200 shadow-2xs shrink-0">
              ...
              <div className="space-y-1.5">
                <span className="text-[11px] font-bold text-slate-700">Auto-Provisioned Tables &amp; Fields:</span>
                <div className="grid grid-cols-2 gap-1.5 text-[10px]">
                  <div className="p-2 rounded-xl bg-sky-50/70 border border-sky-100 min-w-0">
                    <div className="font-bold text-sky-950 truncate">📋 patients</div>
                    <div className="text-slate-500 text-[9.5px] truncate">id, regNo, name, phone, seenBy</div>
                  </div>
                  <div className="p-2 rounded-xl bg-emerald-50/70 border border-emerald-100 min-w-0">
                    <div className="font-bold text-emerald-950 truncate">🔄 followUps</div>
                  ...
                  <div className="p-2 rounded-xl bg-slate-50 border border-slate-200 min-w-0">
                    <div className="font-bold text-slate-800 truncate">👨‍⚕️ locums</div>
                    <div className="text-slate-500 text-[9.5px] truncate">id, name, idNumber, status</div>
                  </div>
                  <div className="p-2 rounded-xl bg-amber-50/70 border border-amber-100 min-w-0">
                    <div className="font-bold text-amber-950 truncate">🏥 doctors &amp; settings</div>
              ...
              <div className="space-y-1.5 text-[11px] font-mono text-slate-600 bg-slate-50 p-3 rounded-2xl border border-slate-200">
                <div className="flex justify-between items-center pb-1 border-b border-slate-200">
                  <span className="text-slate-500">Database Name:</span>
                  <span className="font-bold text-slate-800">{dbStats?.databaseName || 'NamanaPhysioLocalDB'}</span>
                </div>
                <div className="flex justify-between items-center pb-1 border-b border-slate-200">
                  <span className="text-slate-500">Storage Engine:</span>
                  <span className="font-bold text-emerald-700">{dbStats?.engine || 'IndexedDB'} (Native)</span>
                </div>
                <div className="flex justify-between items-center pb-1 border-b border-slate-200">
                  <span className="text-slate-500">Total Patients:</span>
                  <span className="font-bold text-sky-700">{dbStats?.totalPatients ?? patients.length} records</span>
                </div>
                <div className="flex justify-between items-center pb-1 border-b border-slate-200">
                  <span className="text-slate-500">Follow-up Sessions:</span>
                  <span className="font-bold text-sky-700">{dbStats?.totalFollowUps ?? patients.reduce((acc, p) => acc + (p.followUps?.length || 0), 0)} sessions</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-500">Device Status:</span>
                  <span className="font-bold text-emerald-700">100% Offline &amp; Persistent</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-1 mt-auto">
              <button
                ...
                className="flex items-center justify-center gap-1.5 px-3 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-bold transition-colors cursor-pointer disabled:opacity-50"
                title="Verify database health and refresh table indices"
              >
                <RefreshCw className={`w-3.5 h-3.5 text-sky-600 shrink-0 ${isCheckingDb ? 'animate-spin' : ''}`} />
              ...
              <button
                ...
                className="flex items-center justify-center gap-1.5 px-3 py-2.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 rounded-xl text-xs font-bold transition-colors cursor-pointer"
          ...
          <div className="bg-white p-5 sm:p-6 rounded-3xl border border-sky-100 shadow-xs flex flex-col justify-between space-y-4">
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="text-xs font-bold text-sky-600 hover:text-sky-800 hover:underline cursor-pointer shrink-0"
                >
              ...
              <div className="p-4 bg-amber-50/50 border border-amber-200/80 rounded-2xl space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-xs font-bold text-amber-950">Dual Archive Sheets (Strictly Add-Only Permanent Archives)</h4>
                  </div>
                ...
                <div className="space-y-2.5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl text-xs font-mono text-slate-800 placeholder-slate-400 focus:border-amber-500 outline-none shadow-2xs"
                    />
                  </div>
                  ...
                  <div className="pt-1 flex items-center justify-between gap-2">
                    <button
                      ...
                      className="flex items-center justify-center gap-1.5 px-4 py-2 bg-amber-600 hover:bg-amber-700 active:bg-amber-800 text-white rounded-xl text-xs font-bold transition-colors cursor-pointer shadow-xs disabled:opacity-40"
                    >
                    ...
                    {areArchivesConfirmed ? (
                      <span className="text-[11px] font-bold text-emerald-700 flex items-center gap-1">
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> Both Links Confirmed Accessible
                      </span>
                    ) : (
                  ...
                  {archiveNotice && (
                    <div className={`p-2.5 rounded-xl text-xs flex items-start gap-2 ${archiveNotice.type === 'success' ? 'bg-emerald-50 text-emerald-900 border border-emerald-200' : 'bg-rose-50 text-rose-900 border border-rose-200'}`}>
              ...
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-800">Deployment Code &amp; Phone Directory Dashboard</span>
                  </div>
                  {areArchivesConfirmed ? (
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200 flex items-center gap-1">
                      <Unlock className="w-3 h-3 text-emerald-600" /> Code Unlocked
                    </span>
                  ) : (
                ...
                {!areArchivesConfirmed ? (
                  <div className="p-3 bg-white border border-dashed border-slate-300 rounded-xl text-xs text-slate-600 flex items-center justify-between gap-3">
                    ...
                    <button
                      type="button"
                      disabled
                      className="px-3.5 py-1.5 bg-slate-100 text-slate-400 border border-slate-200 rounded-xl text-xs font-bold cursor-not-allowed shrink-0"
                    >
                      Locked
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-bold px-3.5 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-xl cursor-pointer shadow-2xs transition-colors"
                      >
                        {copiedScript ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                        <span>{copiedScript ? 'Code.gs Copied!' : 'Copy Apps Script Code (Code.gs)'}</span>
                      </button>
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-bold px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl cursor-pointer shadow-2xs transition-colors"
                      >
                    ...
                    <div className="p-3 bg-emerald-50/70 border border-emerald-200 rounded-xl text-[11px] text-emerald-950 space-y-1">
                      ...
                      <div className="font-mono text-[10.5px] text-emerald-800 break-all">
                        Archive 1: {archive1Status?.id || settings.archiveSheetId1}<br />
                        Archive 2: {archive2Status?.id || settings.archiveSheetId2}
                      </div>
                    </div>

                    <div className="p-3 bg-amber-50 border border-amber-300 rounded-xl text-[11px] text-amber-950 space-y-1">
                ...
                {showScriptGuide && (
                  <div className="p-3 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 space-y-2 animate-in fade-in duration-200">
                    <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
                      <button
                        type="button"
                        onClick={() => setCodeViewerTab('script')}
                        className={`text-[11px] font-bold px-2.5 py-1 rounded-lg transition-colors cursor-pointer ${codeViewerTab === 'script' ? 'bg-sky-100 text-sky-800' : 'text-slate-500 hover:bg-slate-100'}`}
                      >
                        Step-by-Step Guide
                      </button>
                      <button
                        ...
                        className={`text-[11px] font-bold px-2.5 py-1 rounded-lg transition-colors cursor-pointer ${codeViewerTab === 'html' ? 'bg-emerald-100 text-emerald-800' : 'text-slate-500 hover:bg-slate-100'}`}
                      >
                    ...
                    {codeViewerTab === 'script' ? (
                      <ol className="list-decimal pl-4 space-y-1.5 leading-relaxed text-slate-600 font-medium text-[11px]">
                ...
                <label className="text-xs font-bold text-slate-700 flex items-center justify-between">
                  ...
                  {webhookUrl && webhookUrl.trim().startsWith('http') && (
                    <span className="text-[11px] text-emerald-700 font-semibold flex items-center gap-1">
                      <Zap className="w-3 h-3 text-emerald-600" /> Ready for Direct Dashboard Access
                    </span>
                  )}
                </label>
                <input
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1 mt-auto">
              <button
                ...
                className="flex items-center justify-center gap-1.5 py-2.5 px-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-colors cursor-pointer shadow-xs disabled:opacity-40"
              ...
              >
                ...
                <span className="truncate">Pull via Webhook</span>
              </button>
              ...
              <button
                ...
                className="flex items-center justify-center gap-1.5 py-2.5 px-3 bg-sky-600 hover:bg-sky-700 text-white rounded-xl text-xs font-bold transition-colors cursor-pointer shadow-xs disabled:opacity-40"
              ...
              >
                ...
                <span className="truncate">Push via Webhook</span>
              </button>
              ...
              <button
                ...
                className="flex items-center justify-center py-2.5 px-3 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-bold transition-colors cursor-pointer"
              >
                <span className="truncate">Save Config</span>
              </button>
            </div>
          </div>
          ...
          <div className="bg-white p-5 sm:p-6 rounded-3xl border border-sky-100 shadow-xs flex flex-col justify-between space-y-4">
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold shrink-0 ${settings.autoHourlyPush !== false ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>
              ...
              <div className="flex items-center justify-between p-3 bg-sky-50/60 rounded-2xl border border-sky-100 text-xs">
                <div>
                  <div className="font-bold text-sky-950">Auto Hourly Trigger</div>
                  <div className="text-[11px] text-slate-500">Executes on the hour (:00) &amp; catches up missed hours</div>
                </div>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                    settings.autoHourlyPush !== false ? 'bg-emerald-600' : 'bg-slate-300'
                  }`}
                >
                  <span
              ...
              <div className="space-y-1.5 text-[11px] font-mono text-slate-600 bg-slate-50 p-3 rounded-2xl border border-slate-200">
                <div className="flex justify-between items-center pb-1 border-b border-slate-200">
                  ...
                  <span className="font-bold text-slate-800">Every 1 Hour (:00)</span>
                </div>
                <div className="flex justify-between items-center pb-1 border-b border-slate-200">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-slate-800 truncate max-w-[200px]" title={webhookUrl ? 'Google Sheet (Cloud) + Device DB' : 'Local Device Database'}>
          ...
          <div className="bg-white p-5 sm:p-6 rounded-3xl border border-sky-100 shadow-xs flex flex-col justify-between space-y-4">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-[10px]">
                <div className="p-2 rounded-xl bg-slate-50 border border-slate-200">
                  <div className="font-bold text-slate-800">📊 CSV Spreadsheets</div>
                ...
                <div className="p-2 rounded-xl bg-sky-50/70 border border-sky-100">
                  <div className="font-bold text-sky-950">📦 JSON Master Snapshot</div>
              ...
              <div className="space-y-2 pt-1">
                <button
                  ...
                  className="w-full flex items-center justify-between p-3 bg-slate-50 hover:bg-sky-50 text-slate-800 hover:text-sky-900 border border-slate-200 hover:border-sky-200 rounded-2xl text-xs font-bold transition-all cursor-pointer shadow-2xs"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <FileSpreadsheet className="w-4 h-4 text-emerald-600 shrink-0" />
            ...
            <div className="border-t border-slate-100 pt-3 mt-auto">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 p-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl text-xs font-bold transition-colors cursor-pointer"
              >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2 text-xs text-slate-600">
                <div className="flex justify-between items-center py-1 border-b border-slate-100">
                  ...
                  <span className="font-bold text-slate-800">{patients.length} records</span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-slate-100">
                  <span className="text-slate-400">Connected Sheet ID:</span>
                  <span className="font-mono text-slate-800 text-[11px] truncate max-w-[160px]" title={cleanId || 'None'}>
                    {cleanId || 'None'}
                  </span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-slate-100">
                ...
                <div className="flex justify-between items-center py-1 border-b border-slate-100">
                  ...
                  <span className="font-bold text-emerald-700">Active &amp; Indexed</span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-slate-100">
                  <span className="text-slate-400">Hourly Auto-Backup:</span>
                  <span className={`font-semibold ${settings.autoHourlyPush !== false ? 'text-emerald-700' : 'text-slate-500'}`}>
                    {settings.autoHourlyPush !== false ? 'Active (:00 Trigger)' : 'Disabled'}
                  </span>
                </div>
                <div className="flex justify-between items-center py-1">
                  <span className="text-slate-400">Protected Mode:</span>
                  <span className="font-semibold text-emerald-700">Passkey Authenticated</span>
                </div>
              </div>
            </div>

            {/* Privacy & Security Guarantee Badge */}
            <div className="p-3 bg-emerald-50/70 border border-emerald-200/80 rounded-2xl text-[11px] text-emerald-900 space-y-1 mt-auto">
              <div className="font-bold flex items-center gap-1.5 text-emerald-950">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                <span>Local-First &amp; Zero Cloud Intermediary</span>
              </div>
              <p className="text-emerald-800 leading-relaxed text-[10.5px]">
      ...
      {pendingAction && (
        ...
        >
          ...
          >
            {/* Caution Banner */}
            <div className="bg-gradient-to-r from-rose-600 via-rose-700 to-rose-800 px-6 py-4 text-white flex items-center justify-between">
              ...
              <button
                ...
                className="p-1.5 rounded-lg hover:bg-rose-900/40 text-rose-100 hover:text-white transition-colors cursor-pointer"
            ...
            <form onSubmit={handleConfirmAction} className="p-6 space-y-4">
              ...
              <div>
                <label htmlFor="action-passkey-input" className="block text-xs font-bold text-slate-700 mb-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-10 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm font-mono tracking-wider text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-all"
                  />
                  <button
                    type="button"
              ...
              <div className="pt-2 flex items-center justify-end gap-2.5">
                ...
                <button
                  ...
                  className="px-5 py-2 text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 active:bg-rose-800 rounded-xl shadow-xs transition-colors cursor-pointer"
                >
                  Authorize &amp; Proceed
                </button>
              </div>
